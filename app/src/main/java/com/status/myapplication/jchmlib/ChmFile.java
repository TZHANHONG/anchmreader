...
public class ChmFile {
    ...
    public final HashMap<String, ChmUnitInfo> dirMap = new LinkedHashMap<String, ChmUnitInfo>();
    public String encoding = "UTF-8";
    /**
     * Mapping from paths to titles.
     */
    public HashMap<String, String> pathToTitle = null;
    ...
    private static ChmFile file;

    public static ChmFile getInstance(String homeFile) {
        if (file == null) {
            try {
                file = new ChmFile(homeFile);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private ChmFile(String filename) throws IOException {
        try {
    ...
    public String getPathOfObject(String title) {
        if (pathToTitle == null && tree == null) {
            tree = getTopicsTree();
        }

        if (pathToTitle != null
                && pathToTitle.containsValue(title)) {
            Iterator iterator = pathToTitle.entrySet().iterator();
